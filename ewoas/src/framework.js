...
d$.show = function(id, asBlock) {
	d$(id).style.display = asBlock? "block" : "inline";
...
// converts the number of bytes to a human readable form. Code by FBnil
function _convert_bytes(bytes) {
	var U=['bytes','KB','MB','GB','TB','PB','EB','ZB','YB'];
	var n=0;
...
// implement an sprintf() bare function. TODO: enhance
...
function _get_this_filename() {
	...
	filename = filename.replace(/#.*$/, ""); // remove fragment (if any)
	if (is_windows) {
...
// for(var i=0,b=1;i<32;++i,b<<=1){woas.bitfield._field_mask[i]=b} // Procedural fill of _field_mask
...
woas.ready = function(func,interval){
	if(func){
		if(interval)
			woas.ready.t=interval;
		woas.ready.F.push(func);
		if(woas.ready.x) return false;
		woas.ready.x=1;
	}
	if(document.body){
		while(func = woas.ready.F.shift()){
			func();
		}
		woas.ready.x=0;
		return true;
	}else{
		window.setTimeout( woas.ready, woas.ready.t);
		return false;
	}
}
woas.ready.F =new Array();
woas.ready.t=100; // default interval
woas.ready.x=1; // still active
window.setTimeout(woas.ready, woas.ready.t);

// the condition is a function that must become true to execute the second function.
woas.readyif = function(cond,func,interval){
	if(cond && func){
		if(interval)
			woas.readyif.t=interval;
		woas.readyif.F.push(cond,func);
		if(woas.readyif.x) return false;
		woas.readyif.x=1;
	}
	if(document.body){
		var T=new Array();
		while((cond = woas.readyif.F.shift()) && (func = woas.readyif.F.shift())){
			if(cond())func();
			else T.push(cond,func);
		}
		if(T.length>-1)
			woas.readyif.F=T;
		else
	...
	if(woas.readyif.x){
		window.setTimeout( woas.readyif, woas.readyif.t);
		return false;
	}
	return true;
}
woas.readyif.F =new Array();
woas.readyif.t=250; // default interval
woas.readyif.x=1; // still active
window.setTimeout(woas.readyif, woas.readyif.t);
