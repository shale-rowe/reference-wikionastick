	...
	load: function(s) {
		...
		s.replace(this.reHotkeys, function(str, hkey, lambda, binding) {
			...
			if (hkey === "CUSTOM") {
				...
				woas.log("CUSTOM:" + lambda +binding);
			} else {
...
// TODO: rename to woas.plugins.edit ?
...
//		alert(M[1]+";"+M[2]+";"+M[3]+";"+M[4]+";")
		// if double colon declaration was found, then process
		if (M) {
			var fn = M[2];
			...
			// macro definition request
			if (M[1] === '%') {
				// when macro is not defined, define it
				if (this.create(fn, M[3], M[4])) {
...
//						alert("EXEC(woas.macro.functions["+fi+"])"+"(macro,"+M[3].substr(1,pl-2)+");"+M.text)
						rv = eval( "(woas.macro.functions["+fi+"])"+
									"(macro,"+M[3].substr(1,pl-2)+");"
							);
//						alert("RV|"+rv)
					}
				}
		...
		}else{ // not the macro syntax, so we use a default
				var _print="";
				...
				try{
					...
					macro.text = woas._make_preformatted(macro.text, "color:red;font-weight:bold");
				}
		}
		return macro;
	},
