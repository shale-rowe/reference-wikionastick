...
woas.cmd_copy_page = function() {
	var pname = this._new_page("Insert copied page title:", true, current+" (copy)");
	if (pname === null)
		return;
...
woas.cmd_new_page = function(name) {
	this._new_page(this.i18n.INSERT_NEW, false, name||'');
};
...
// JavaScript shortcuts (and their corresponding JavaScript commands) for special pages
woas.shortcuts = [
	"All Pages",
	"Backlinks",
	"Dead Pages",
...
woas.shortcuts_js = [
	...
	"cmd_main_page",
	"cmd_new_page",
	"special_orphaned_pages",
	"special_recent_changes"
	];

woas.unexportable_pages = [
	"Backlinks",
	"Delete Page",
	"Copy page",
	"Edit CSS",
...
// TODO: speed test return s.split.forEach(function(c){return c.charCodeAt(0)}); Also: used only once! Make inline
...
// return byte_arr.forEach(function(b){return String.fromCharCode(b)}).join("");
...
function Function2String(F){
  return F.toString().replace(/^[^\{]+\{\s+/,"").replace(/[;\s\n]*\}.*?$/,"");
}
function Add2Function(F,code,prepend,nodotcomma){
	if(F===undefined){
		F=Function(code);
		return F;
	}
	code=Function2String(Function(code));
	if(F.toString().indexOf(code)==-1){
		var P = F.toString().match(/^function ?(\S*)\((.*?)\)/);
		var S = prepend? code+"\n"+Function2String(F) : Function2String(F)+(nodotcomma?"\n":";\n")+code;
		F = (P===null)? Function(S) : Function(P[2], S);
	};
	return F
}


// inside a page use: [[Javascript::woas.ShowPageSource()]]
woas["ShowPageSource"] = function(){
	var w = woas.popup("Page Source", 450, 200, ",menubar=no,toolbar=no,location=yes,status=yes,dialog=yes");
	w.document.writeln(woas.xhtml_encode(d$("woas_wiki_area").innerHTML));
	w.document.close();
}