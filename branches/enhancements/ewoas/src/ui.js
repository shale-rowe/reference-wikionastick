...
// And inside a script:
// woas.ready( function(){
// d$('char').onkeydown=function(e){var e=window.event || e; document.getElementById("keycode").value=e.keyCode;}
// })
...
keyindex=function(keycode,meta,mode){return keycode + (meta<<8) + (mode<<12)};

KEYS = []; // Here be keys.
	// Escape pressed during edit mode
	KEYS[keyindex(27,0,1)] = function(){woas.ui.cancel();ff_fix_focus();}
	// Pressing F2 while viewing will edit the  page
	KEYS[keyindex(113,0,0)] = function(){woas.ui.edit();} 
	// Pressing F2 while editing will save your page
	KEYS[keyindex(113,0,1)] = function(){woas.save();}
	// Pressing F2 while viewing / editing a form will edit the  page
	KEYS[keyindex(113,0,3)] = function(){woas.ui.edit();} 
	// Pressing SHIFT+F2 while editing will quicksave the page. (the current page is saved as wsif, and can be imported if woas crashes before you can save)
	// the idea is to make a plugin that will autosave each x minutes 
	KEYS[keyindex(113,1,1)] = function(){woas._quicksave();}

	
woas._quicksave = function(filename,pagename,pagecontent){
	if(!woas.ui.edit_mode){log("cannot Quicksave");return false;}
	...
	log("autosave:"+filename+" "+pagename);
	var result = woas.save_file(woas.ROOT_DIRECTORY+filename, woas.file_mode.UTF8_TEXT,woas._create_wsif("quicksave", pagename, woas.utf8.do_escape(pagecontent), "8bit/plain")  );
	woas.progress_finish();
	if (!result){
		var msg="Quicksave failed!";
		alert(msg);
		log(msg);
		return false;
	}
...
woas._create_wsif = function(author, pagename,pagecontent, encoding){
	...
	// KEYS[keyindex(woas.hotkey.all.back,0,0)] = function(){go_back();ff_fix_focus();return false;} // 
	// KEYS[keyindex(woas.hotkey.all.cancel,0,0)] = function(){go_back();ff_fix_focus();return false;} // 

	// Pressing F4 will bring up the search
	KEYS[keyindex(115,0,0)] = function(){woas.go_to('Special::Search')} 
	// Pressing SHIFT+F2 will create a new subpage
	KEYS[keyindex(113,1,0)] = function(){woas.cmd_new_page(current.replace(/::$/,'')+'::');}
	// Pressing SHIFT+F1 will goto TODO
	KEYS[keyindex(112,1,0)] = function(){woas.go_to('TODO')} 

	KEYS[keyindex(119,1,1)] = function(){FullTagThis('big');} // Shift+F8 in Editor!
	KEYS[keyindex(9,1,0)] = function(){alert('TAB!'); return false;}
  
	KEYS[keyindex(119,1,0)] = function(){alert(getSelectedText());} // Shift+F8 in viewer!

	KEYS[keyindex(80,0,0)] = function(){woas.go_to('WoaS::Plugins')} // press P for plugin
	KEYS[keyindex(81,0,0)] = function(){woas.go_to('WoaS::Plugins::NewPageTemplate');woas.ui.edit(); return false;} // press Q for plugin edit
	// KEYS[keyindex(80,1,0)] = function(){woas.log("AUTOS");woas.go_to('WoaS::Plugins::Autosave');woas.readyif(function(){ woas.log(current); return current =='WoaS::Plugins::Autosave'}, woas.ui.edit);} // press shift P for pluging edit
...
woas.ui = {
	...
	event: undefined, // store keypress event  ( woas.ui.event.keyCode )
	META:0, // meta keys bitfield 1=SHIFT 2=CTRL 4=ALT
	KEY: undefined, // the pressed key
	// n is trueish for KEYDOWN and falsish for KEYUP
	getkey: function(evnt,n){
		...
		switch(key){
			...
			default: this.KEY = n? key : false;
		}
		// Fix bug in FF. keyup for AltGR only generates 1 event. (keydown generates 2 events, for CTRL and one for ALT)
		if(this.META & 4 && !evnt.altKey){this.META&= 3;if(this.edit_mode)this.button.setMETA(this.META);}

//		log("kbd "+(n?"DOWN ":"UP ")+this.META+" "+this.KEY+" "+key+
//		" evnt=" + evnt.which+ " alt="+ evnt.altKey + " ctrl="+ evnt.ctrlKey  + " shift="+ evnt.shiftKey  + " meta="+ evnt.metaKey+ "   "+ evnt.timeStamp);
		// " _textbox_focus="+this._textbox_focus+" edit_mode="+this.edit_mode+" search_focused="+(current == "Special::Search" && this._textbox_focus) );
		document.title=document.title.replace(/(--\d+)?$/, "--"+this.META); // DEBUGGING
		return this.KEY;
	},
	
	// helper function for keypresses
//	key_is: function(k,m){
//		if(this.KEY==k&&this.META==(m||0))
//			return 1;
//	},

	// resets the woas.ui.META value (required when you press CTRL+F to find things in mozilla, then the META never unpresses, due to lost focus)
	kbd_blur: function(){this.META=0;document.title=document.title.replace(/(--\d+)?$/, "--"+this.META);},
	
	// On key up
	onKeyUp_event_hook: function(orig_e) {
		woas.ui.getkey(orig_e,0);
	...
	onKeyDown_event_hook: function(orig_e) {
		...
		if(!e) return orig_e;
		var Mode = woas.ui.edit_mode? 1 : (woas.ui._textbox_focus&&(current == "Special::Search"))? 2: woas.ui._textbox_focus? 3 : 0;
